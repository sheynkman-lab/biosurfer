    ...
    def load_gencode_gtf(self, gtf_file: str, overwrite=False) -> None:
        ...
        with open(gtf_file) as gtf:
            ...
            for i, line in enumerate(t, start=1):
                ...
                if i % CHUNK_SIZE == 0:
                    update_and_insert(session, Gene, genes_to_update, genes_to_insert)
                    update_and_insert(session, GencodeTranscript, transcripts_to_update, transcripts_to_insert)
            update_and_insert(session, Gene, genes_to_update, genes_to_insert)
            update_and_insert(session, GencodeTranscript, transcripts_to_update, transcripts_to_insert)
        session.commit()
        ...
        for i, (transcript_id, exon_list) in enumerate(t):
            ...
            if i % CHUNK_SIZE == 0:
                update_and_insert(session, GencodeExon, exons_to_update, exons_to_insert)
        update_and_insert(session, GencodeExon, exons_to_update, exons_to_insert)
        session.commit()
        ...
        for i, (transcript_id, cds_list) in enumerate(t):
            ...
            if i % CHUNK_SIZE == 0:
                update_and_insert(session, ORF, orfs_to_update, orfs_to_insert)
        update_and_insert(session, ORF, orfs_to_update, orfs_to_insert)
        session.commit()
    ...
    def load_pacbio_gtf(self, gtf_file: str, overwrite=False) -> None:
        ...
        with open(gtf_file) as gtf:
            ...
            for i, line in enumerate(t, start=1):
                ...
                if i % CHUNK_SIZE == 0:
                    update_and_insert(session, Gene, genes_to_update, genes_to_insert)
                    update_and_insert(session, PacBioTranscript, transcripts_to_update, transcripts_to_insert)
            update_and_insert(session, Gene, genes_to_update, genes_to_insert)
            update_and_insert(session, PacBioTranscript, transcripts_to_update, transcripts_to_insert)
        session.commit()
        ...
        for i, (transcript_id, exon_list) in enumerate(t):
            ...
            if i % CHUNK_SIZE == 0:
                update_and_insert(session, PacBioExon, exons_to_update, exons_to_insert)
        update_and_insert(session, PacBioExon, exons_to_update, exons_to_insert)
        session.commit()
    ...
    def load_transcript_fasta(self, transcript_fasta: str, id_extractor: Callable[[str], 'FastaHeaderFields'], id_filter: Callable[[str], bool] = lambda x: False):
        ...
        for record in t:
            ...
            if len(transcripts_to_update) == CHUNK_SIZE:
                session.bulk_update_mappings(Transcript, transcripts_to_update)
                transcripts_to_update[:] = []
        session.bulk_update_mappings(Transcript, transcripts_to_update)
            ...
            if transcript_id in existing_orfs:
                ...
                for start, stop in orfs:
                    if stop - start + 1 == (seq_length + 1)*3:
                        ...
                        break
            if i % CHUNK_SIZE == 0:
                update_and_insert(session, ORF, orfs_to_update, [])
                update_and_insert(session, Protein, proteins_to_update, proteins_to_insert)
        update_and_insert(session, ORF, orfs_to_update, [])
        update_and_insert(session, Protein, proteins_to_update, proteins_to_insert)  
        session.commit()
    ...
    def load_sqanti_classifications(self, sqanti_file: str):
        ...
        with open(sqanti_file) as f:
            ...
            for row in tqdm(reader, desc='Reading SQANTI classifications', unit='lines'):
                ...
                if len(transcripts_to_update) == CHUNK_SIZE:
                    session.bulk_update_mappings(PacBioTranscript, transcripts_to_update)
                    transcripts_to_update[:] = []
            session.bulk_update_mappings(PacBioTranscript, transcripts_to_update)
            session.commit()
...
def update_and_insert(session, mapper, mappings_to_update, mappings_to_insert):
