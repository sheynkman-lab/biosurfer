...
gene_id_mapper = dict()
with db.get_session() as session:
    for old_gene_id in sqtls['gene_id'].unique():
        new_gene_id = session.execute(
            select(Gene.accession).
            where(Gene.accession.startswith(stem(old_gene_id)))
        ).scalar()
        if new_gene_id:
            gene_id_mapper[old_gene_id] = new_gene_id
...
def get_augmented_sqtl_record(row):
    ...
    with db.get_session() as session:
        ...
        if force_plotting or not os.path.isfile(fig_path):
            isoplot = IsoformPlot(
                gc_transcripts + [None, None] + containing + [None, None] + lacking + [None, None, None],
                columns = {
            ...
            with ExceptionLogger(f'Error plotting {gene.name} {junc}'):
                ...
                isoplot.draw_region(type='line', color='k', linewidth=1, track=len(gc_transcripts) + 0.5, start=gene.start, stop=gene.stop)
                isoplot.draw_region(type='line', color='k', linestyle='--', linewidth=1, track=len(gc_transcripts) + len(containing) + 2.5, start=gene.start, stop=gene.stop)
